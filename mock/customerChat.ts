...
const buildPrompt = (body: any) => {
  ...
  return [
    ...
    '- DỮ LIỆU THỰC ĐƠN LÀ NGUỒN SỰ THẬT: Khi nói về món ăn, đồ uống, giá, ưu đãi hoặc tình trạng còn món, chỉ được dùng đúng các món nằm trong "THỨC ĂN HÔM NAY", "ĐỒ UỐNG HÔM NAY" và "ƯU ĐÃI HÔM NAY". Tuyệt đối không tự bịa trà đào, cà phê, nước, món ăn hoặc ưu đãi không có trong dữ liệu.',
    '- KHÔNG TỰ MÂU THUẪN: Nếu trước đó đã nói hôm nay không có một danh mục nào, khi khách nhắn ngắn như "có", "ok", "được" thì không được đổi ý và bịa món mới. Hãy gợi ý món thật đang có trong danh sách.',
    '- NẾU KHÔNG CÓ DỮ LIỆU: Hãy nói nhẹ nhàng là hiện chưa thấy món đó trong thực đơn hôm nay, rồi đề xuất các món thật đang có. Không nhắc lỗi hệ thống hoặc danh mục nội bộ với khách.',
